...
def btls_server_cert(tmpdirname, server_log_file, curve, psk=False):
	...
	if psk:
		cmd = ('s_server -port 8084 -state -key {} -cert {} -tls1_2 -psk 123456 -psk_hint 123  >> {}'
				.format(priv, cert, server_log_file))
	else:
		cmd = ('s_server -port 8084 -state -key {} -cert {} -tls1_2 >> {}'
				.format(priv, cert, server_log_file))
...
def btls_client_cert(client_log_file, curve, ciphersuites, psk=False):
	for ciphersuite in ciphersuites:
		if psk:
			cmd = ('s_client -connect 127.0.0.1:8084 -verify 1 -state -cipher {} -tls1_2 -psk 123456 2>{}'
					.format(ciphersuite, client_log_file))
		else:
			cmd = ('s_client -connect 127.0.0.1:8084 -verify 1 -state -cipher {} -tls1_2 2>{}'
					.format(ciphersuite, client_log_file))
