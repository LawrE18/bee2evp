...
def btls_server_cert(tmpdirname, server_log_file, curve, psk=False):
	...
	if psk:
		cmd = ('s_server -port 443 -key {} -cert {} -tls1_2 -psk 123456 -psk_hint 123  >> {}'
				.format(priv, cert, server_log_file))
	else:
		cmd = ('s_server -port 443 -key {} -cert {} -tls1_2 >> {}'
				.format(priv, cert, server_log_file))
...
def btls_client_cert(client_log_file, curve, ciphersuites, psk=False):
	for ciphersuite in ciphersuites:
		if psk:
			cmd = ('s_client -connect 127.0.0.1:443 -cipher {} -tls1_2 -psk 123456 2>{}'
...
def btls_server_nocert(server_log_file):
	cmd = ('s_server -port 443 -tls1_2 -psk 123456 -psk_hint 123 -nocert >> {}'
			.format(server_log_file))
...
def btls_client_nocert(client_log_file, curves_list, ciphersuites):
	for ciphersuite in ciphersuites:
		for curves in curves_list:
			if curves != 'NULL':
				cmd = ('s_client -connect 127.0.0.1:443 -cipher {} -tls1_2 -curves {} -psk 123456 2>{}'
